...
def get_async_router(
    ...
    async def run_agent_or_team_or_workflow(
        ...
        if workflow_id and workflows:
            ...
            try:
                parsed_workflow_input: dict = json.loads(workflow_input)
        ...
        if stream:
            if agent:
                ...
                return StreamingResponse(
                    (json.dumps(asdict(result)) for result in await workflow_instance.arun(**parsed_workflow_input)),
                    media_type="text/event-stream",
                )
        else:
            if agent:
                ...
                workflow_instance.user_id = user_id
                workflow_instance.session_name = None
                return (await workflow_instance.arun(**parsed_workflow_input)).to_dict()
