...
from typing import Any, AsyncContextManager, Dict, List, Literal, Optional, Union, cast
        ...
        for server_params in self.server_params_list:
            ...
            if isinstance(server_params, StdioServerParameters):
                ...
                session = await self._async_exit_stack.enter_async_context(
                    cast(AsyncContextManager[Any], ClientSession(read, write, read_timeout_seconds=float(self.timeout_seconds)))
                )
            ...
            # Handle SSE connections
                ...
                session = await self._async_exit_stack.enter_async_context(
                    cast(AsyncContextManager[Any], ClientSession(read, write))
                )
                await self.initialize(session)
